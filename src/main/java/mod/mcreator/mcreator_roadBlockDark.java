...
public class mcreator_roadBlockDark extends theworlditems2.ModElement {
    public static Block block = new BlockCustom().setHardness(5.0f).setResistance(8.0f).setLightLevel(0.0f).setUnlocalizedName("roadblockdark").setLightOpacity(255).setCreativeTab(CreativeTabs.BUILDING_BLOCKS);

    @Override
    public void preInit(FMLPreInitializationEvent event) {
        block.setHarvestLevel("pickaxe", 4);
        block.setRegistryName("roadblockdark");
    ...
    public void load(FMLInitializationEvent event) {
        if (event.getSide() == Side.CLIENT) {
            Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(Item.getItemFromBlock(block), 0, new ModelResourceLocation("theworlditems2:roadblockdark", "inventory"));
        }
    }
