...
public class mcreator_roadBlockRed extends theworlditems2.ModElement {
    public static Block block = new BlockCustom().setHardness(5.0f).setResistance(8.0f).setLightLevel(0.0f).setUnlocalizedName("roadblockred").setLightOpacity(255).setCreativeTab(CreativeTabs.BUILDING_BLOCKS);

    @Override
    public void preInit(FMLPreInitializationEvent event) {
        block.setHarvestLevel("pickaxe", 2);
        block.setRegistryName("roadblockred");
    ...
    public void load(FMLInitializationEvent event) {
        if (event.getSide() == Side.CLIENT) {
            Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(Item.getItemFromBlock(block), 0, new ModelResourceLocation("theworlditems2:roadblockred", "inventory"));
        }
    }
