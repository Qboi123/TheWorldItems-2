...
public class mcreator_gateHell extends theworlditems2.ModElement {
    @Override
    public void generateSurface(World world, Random random, int i2, int k2) {
        if (random.nextInt(1000000) + 1 <= 6000) {
            ...
            if (Objects.requireNonNull(Biome.REGISTRY.getNameForObject(biome)).getResourcePath().equals("desert")) {
                biomeCriteria = true;
            }
            if (!biomeCriteria) {
                return;
            ...
            Template template = ((WorldServer)world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(), new ResourceLocation("theworlditems2", "ld_portal_001_hell"));
