...
public class mcreator_roadBlockFine extends theworlditems2.ModElement {
    public static Block block = new BlockCustom().setHardness(8.0f).setResistance(12.0f).setLightLevel(0.0f).setUnlocalizedName("roadblockfine").setLightOpacity(255).setCreativeTab(CreativeTabs.BUILDING_BLOCKS);

    @Override
    public void preInit(FMLPreInitializationEvent event) {
        block.setHarvestLevel("pickaxe", 4);
        block.setRegistryName("roadblockfine");
    ...
    public void load(FMLInitializationEvent event) {
        if (event.getSide() == Side.CLIENT) {
            Minecraft.getMinecraft().getRenderItem().getItemModelMesher().register(Item.getItemFromBlock(block), 0, new ModelResourceLocation("theworlditems2:roadblockfine", "inventory"));
        }
    }
